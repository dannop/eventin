...
class FederationsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create federation" do
    assert_difference('Federation.count') do
      post federations_url, params: { federation: { name: @federation.name, state: @federation.state } }
    end
  ...
  test "should update federation" do
    patch federation_url(@federation), params: { federation: { name: @federation.name, state: @federation.state } }
